    ...
    if status is True:
        try:
            await commands.add_message(
                message.from_user.id, message.message_id, message.message_id
            )
            ...
            for user in users:
                try:
                    ...
                    if message.reply_to_message:
                        replied_id = await commands.get_replied_message(
                    ...
                    await commands.add_message(
                        user.user_id, message.message_id, copied_message.message_id
                    )
                except Exception:
                    print("Copying error")
